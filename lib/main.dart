...
TimeData calculateTime() {
  ...
  if (now.isBefore(targetTime)) {
    ...
    if (totalMinutes > 1000) {
      // Jeśli od 6:15 wczoraj minęło więcej niż 1000 min, liczymy w dół do 6:15 dzisiaj
      difference = targetTime.difference(now);
      title = "Zostało do 6:15";
      data = "${difference.inMinutes} min";
    } else {
      // Jeśli nie, nadal liczymy w górę od 6:15 wczoraj
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    super.initState();
    // Inicjalizacja i pierwsza aktualizacja widżetu
    _updateWidget();
    // Uruchomienie timera dla UI aplikacji
    _startTimer();
  }

  void _startTimer() {
    // Aktualizuj UI co sekundę
    _updateTime(); // Pierwsze wywołanie
  ...
  // Funkcja do ręcznej aktualizacji widżetu
